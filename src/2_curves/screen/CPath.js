...
import { getAbsolutePoint, partial, pipe } from "../../helpers/utils";
import { ABS_HEIGHT, ABS_WIDTH, STEP } from "./config";

const abs = getAbsolutePoint(ABS_WIDTH / 2, ABS_HEIGHT / 2, STEP);
const shiftXorY = ([x, y], shiftX, shiftY) => [x + (shiftX ? .5 : 0), y + (shiftY ? .5 : 0)];

export default class CPath extends Path {
  constructor({ thickness = 2, color = "black", shouldApplyLinear = false } = {}) {
